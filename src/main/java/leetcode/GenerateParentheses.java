...
public class GenerateParentheses {
    ...
    private void directedGenParenthesis(String parenthesis, int numsRightNeeded,
                                        int numsLeftNeeded, List<String> result) {
        if (numsLeftNeeded == 0 && numsRightNeeded == 0) {
            result.add(parenthesis);
            return;
        }
        if (numsLeftNeeded > 0) {
            directedGenParenthesis(parenthesis + "(", numsRightNeeded,
