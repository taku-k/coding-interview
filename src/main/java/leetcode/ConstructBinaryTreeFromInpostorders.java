...
public class ConstructBinaryTreeFromInpostorders {
    ...
    private TreeNode rec(Map<Integer, Integer> nodeToInorderIdx, int[] postorder,
        ...
        node.left = rec(nodeToInorderIdx, postorder, postorderStart,
                postorderEnd - rightTreeSize - 1, inorderStart, rootIdx);
        node.right = rec(nodeToInorderIdx, postorder, postorderEnd - rightTreeSize - 1,
                postorderEnd - 1, rootIdx + 1, inorderEnd);
        return node;
    }
}
